...
public class Game implements ActionListener, MouseListener, KeyListener
{
	...
	public void repaint(Graphics g)
	{
		...
		if (!started)
		{
			g.drawString("Hello " + player_name + " !" , 75, 100 );
		...
		if (gameOver)
		{
			...
			g.drawString("Your score :"+ s  , 100, HEIGHT / 2 + 150);
			String max_score=String.valueOf(max_sc);
			
			try {
			FileWriter myWriter = new FileWriter(player_name);
      			myWriter.write(max_score);
      			myWriter.close();
   			 } catch (IOException e) {
		...
		if (!gameOver && started)
		{
			...
			g.drawString(String.valueOf(max_sc), WIDTH / 2 - 25, 165);
		}
	}
